...
#* 
#* @apiDescription This API contains sensitive data, the client does not want to share this data
#* but does want a consultant to build a health economic model using it, and wants that consultant to be 
#* able to run the model for various inputs (while holding certain inputs fixed and leaving them unknown).

#* Run the DARTH model
#* @serializer csv
#* @param path_to_psa_inputs is the path of the csv
#* @param model_functions gives the github repo to source the model functions from
#* @param param_updates gives the parameter updates to be run
#* @post /runDARTHmodel
function(path_to_psa_inputs = "parameter_distributions.csv",
         model_functions = "https://raw.githubusercontent.com/BresMed/plumberHE/main/R/darth_funcs.R",
         param_updates = data.frame(parameter = c("p_HS1","p_S1H"),
