---
title: Living HTA, Automating HTA with R
author:
  - name: Robert A Smith
    affil: 
    - 1 
...
Two other bodies of work are particularly relevant. The first is the OpenSafely initiative, which inspired this work. @williamson2020factors describe the OpenSafely interface [https://opensafely.org/](https://opensafely.org/), which was developed to analyse electronic health records data without the need to share confidential patient information:
...
* An API, hosted on the company or data provider side. It can be generated using the R package plumber. An automated workflow is created. This workflow sends the economic model to the company API. The model is then run within the company server. The results are sent back to the consultant, and a (PDF) report is automatically generated using RMarkdown. This API server hosts all sensitive data, so that data does not have to be provided to other project partners.

* All of these processes can be controlled with a web-based user-interface. We provide an example user-interface built in the R shiny package [@shiny2021], based on the tutorial application in our previous paper [@smith2020making]. This application allows users to select input parameters with which to query the API, and view the results. This allows non-technical stakeholders to interact with the model in real time, while allowing the company to retain control of the data. The application will always reflect the data on the company server, and the model hosted by the consultant at the time of use.
...
All of the methods  discussed in this paper, as well as the code for the demonstration app can be found contained within an open access GitHub repository  [https://github.com/RobertASmithBresMed/plumberHE](https://github.com/RobertASmithBresMed/plumberHE).

### 1. The economic model

This model code has been adapted from the DARTH group's open source Cohort state-transition model (the Sick-Sicker Model) which is discussed in @alarid2021tutorial with open source code available online [@fernando_alarid_escudero_2022_6400119]. The code includes several functions, but for the purpose of this example we can treat the model as a black box, as a single function called *run_model* which runs the DARTH Sick Sicker model. The *run_model* function takes a single argument, *psa_inputs*, which is a data-frame containing Probabilistic Sensitivity Analysis parameter inputs for the model variables that are allowed to vary. 
...
* in supporting programmatic access.  In contrast to what web applications offer (for example shiny apps), APIs allow users to access data, or other utilities (for example, proprietary applications) programmatically. Programmatic access enables user programs to invoke actions through an application or third-party tool. For example, R users can write a function that fetches or analyses data via an API and use this function in their workflow as any other user-defined function.
...
#### 3.1 Interact with API from RScript
...
#### 3.2 Living HTA - scheduling model runs
...
Source code is available on GitHub at this link: *[https://github.com/RobertASmithBresMed/plumberHE](https://github.com/RobertASmithBresMed/plumberHE)**. 

Archived source code at the time of publication can be found at **10.5281/zenodo.6556887**.
